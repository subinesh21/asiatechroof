...
export default function RelatedLinks() {
  ...
  return (
    <div className="bg-[#F3F4F6] p-12 border-t-4 border-[#C9A84C] mt-20">
      <h3 className="text-[#111827] text-[32px] mb-8" style={{ fontFamily: 'var(--font-bebas, sans-serif)' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-y-4 gap-x-10">
        {landingPages.map((page) => (
          <Link 
            key={page.href} 
            href={page.href}
            className="text-[#111827] hover:text-[#C9A84C] transition-colors text-[14px] no-underline flex items-center gap-2"
          >
            <span className="text-[#C9A84C]">→</span> {page.title}
          </Link>
        ))}
        <Link href="/blog" className="text-[#111827] hover:text-[#C9A84C] transition-colors text-[14px] no-underline flex items-center gap-2">
          <span className="text-[#C9A84C]">→</span> Expert Resource Blog
        </Link>
        <Link href="/" className="text-[#111827] hover:text-[#C9A84C] transition-colors text-[14px] no-underline flex items-center gap-2">
