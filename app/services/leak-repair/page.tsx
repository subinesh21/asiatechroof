        ...
        <div className="grid grid-cols-2 gap-20 items-center">
          <Reveal>
            <div className="h-[560px] bg-cover bg-center relative" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80')" }}>
              <div className="absolute bottom-0 left-0 bg-[#C9A84C] text-[#0D0F12] px-5 py-2.5 text-[10px] font-bold tracking-[2px] uppercase">Leak Repair Experts</div>
            </div>
          </Reveal>
          <Reveal delay={1}>
            ...
            <div className="text-[#F0EDE6] mb-5" style={{ fontFamily: 'var(--font-bebas, sans-serif)', fontSize: '56px' }}>LEAK REPAIR</div>
            <p className="text-[15px] text-[#8A8F9E] leading-[1.9] mb-8">Water leaks can escalate quickly if left unaddressed. Our team provides rapid response, accurate diagnosis using moisture detection tools, and durable repairs that stop leaks from returning.</p>
            <p className="text-[15px] text-[#8A8F9E] leading-[1.9] mb-8">We trace leaks to their source — not just the visible symptoms — ensuring the repair addresses the actual problem and delivers lasting protection.</p>
            <ul className="list-none mb-9">
              {bullets.map((b, i) => (
                <li key={i} className="text-[13px] text-[#8A8F9E] py-2.5 border-b border-[rgba(201,168,76,0.2)] flex gap-3 items-start"><span className="text-[#C9A84C] flex-shrink-0">→</span>{b}</li>
              ))}
            </ul>
